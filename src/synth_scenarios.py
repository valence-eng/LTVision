...
class BaseScenario():
    ...
    def _get_conditional_purchase_prob(self, data: pd.DataFrame, trials: int=None) -> pd.DataFrame:
        """
    ...
    def get_revenue_events(self, customer_data: pd.DataFrame, events_data: pd.DataFrame) -> pd.DataFrame:
        """
        ...
        data['converted'] = self._get_conversion_prob(data) # probability of user ever converting 
        ...
        data['event_probability'] = data.apply(
            lambda x: self.event_decay_scale/x['total_revenue_events']*np.exp(-(self.event_decay_scale/x['total_revenue_events'])*x['days_since_registration']), 
            axis=1)
        data['events_number'] = self._get_conditional_purchase_prob(data, data['total_revenue_events']) # probability of making a purchase conditional to returning
        ...
        data['value'] = data['converted'] * data['events_number'] * data['event_value']
        data = data.drop(['converted', 'events_number', 'event_value', 'total_revenue_events', 'event_probability'], axis=1)
        return data
